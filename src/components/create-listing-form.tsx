...
const BILLS_STATUSES: BillsStatus[] = ["INCLUDED", "EXCLUDED", "PARTIAL"];
...
export function CreateListingForm({
  ...
  const publishListing = async (values: CreateListingFormValues, currentUser: User) => {
    ...
    try {
      ...
      formData.set("aboutText", values.aboutText ?? "");
      formData.set("depositReturnPolicy", values.depositReturnPolicy);
      formData.set("contractAvailable", String(values.contractAvailable));
      formData.set("billsStatus", values.billsStatus);
  ...
  const onSubmit = async (values: CreateListingFormValues) => {
    ...
    // currentUser is guaranteed signed in past this point -- publish
    // straight away, no phone-verification step.
  ...
  return (
    ...
    >
      ...
      <div className="bg-card flex flex-col gap-3 rounded-xl border p-4">
        <h2 className="font-semibold">{t("rulesSectionHeading")}</h2>

        <div className="flex flex-col gap-1.5">
          <Label htmlFor="aboutText">{t("aboutLabel")}</Label>
          <textarea
            id="aboutText"
            {...register("aboutText")}
            className="border-input min-h-24 rounded-md border px-3 py-2 text-sm"
          />
          {errors.aboutText && (
            <p className="text-destructive text-sm">{errors.aboutText.message}</p>
          )}
        </div>

        <div className="flex flex-col gap-1.5">
          <Label htmlFor="depositReturnPolicy">{t("depositReturnPolicyLabel")}</Label>
          <textarea
            id="depositReturnPolicy"
            {...register("depositReturnPolicy")}
            className="border-input min-h-20 rounded-md border px-3 py-2 text-sm"
          />
          {errors.depositReturnPolicy && (
            <p className="text-destructive text-sm">{errors.depositReturnPolicy.message}</p>
          )}
        </div>
        ...
        <div className="flex items-start gap-2">
          <Checkbox
            ...
            checked={watch("contractAvailable")}
            onCheckedChange={(checked) => setValue("contractAvailable", checked === true)}
          />
          <Label htmlFor="contractAvailable" className="text-sm leading-snug font-normal">
            {t("contractAvailableLabel")}
          </Label>
        </div>

        <div className="flex flex-col gap-1.5">
          <Label htmlFor="billsStatus">{t("billsLabel")}</Label>
          <Select
            defaultValue="EXCLUDED"
            onValueChange={(value) => {
              if (value) setValue("billsStatus", value as BillsStatus);
            }}
          >
            <SelectTrigger id="billsStatus" className="w-full">
              <SelectValue>{(value: BillsStatus) => t(`billsStatus.${value}`)}</SelectValue>
            </SelectTrigger>
            <SelectContent>
              {BILLS_STATUSES.map((status) => (
                <SelectItem key={status} value={status}>
                  {t(`billsStatus.${status}`)}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
